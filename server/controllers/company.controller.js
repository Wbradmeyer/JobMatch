    ...
    loginCompany: async (req, res) => {
        try{
            ...
            if(company){
                ...
                if (passwordsMatch){
                    const companyToken = jwt.sign({_id: company._id, email:company.email}, SECRET, {expiresIn:'96h'})
                    res.status(201).cookie('companyToken', companyToken, {httpOnly:true, maxAge: 2 * 60 * 60 * 1000}).json(company)
                }
                else{
